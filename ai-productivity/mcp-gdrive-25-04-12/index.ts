...
async function loadCredentialsAndRunServer() {
  ...
  const oauthKeys = JSON.parse(fs.readFileSync(oauthKeysPath, "utf-8")).installed;
  ...
  auth.on("tokens", (tokens) => {
    if (tokens.refresh_token) {
      credentials.refresh_token = tokens.refresh_token; // Update refresh token if provided
    }
    credentials.access_token = tokens.access_token;
    credentials.expiry_date = tokens.expiry_date;
    fs.writeFileSync(credentialsPath, JSON.stringify(credentials, null, 2)); // Save updated credentials
  });
